...
class DominoSpaceTest(unittest.TestCase):
  ...
  def testCatDomino(self):
    ...
    expected = [
      [PostCorrespondenceState(("", ""),[1]), PostCorrespondenceState(("", ""),[1, 2]), 
        PostCorrespondenceState(("", ""),[2, 3]), PostCorrespondenceState(("", ""),[3, 4])],
      [PostCorrespondenceState(("b", ""),[1]), PostCorrespondenceState(("b", ""),[1, 2]), 
        PostCorrespondenceState(("c", ""),[2, 3]), PostCorrespondenceState(("d", ""),[3, 4])],
      [PostCorrespondenceState(("", "c"),[1]), PostCorrespondenceState(("", "c"),[1, 2]), 
        PostCorrespondenceState(("", "b"),[2, 3]), PostCorrespondenceState(("", "d"),[3, 4])],
      [PostCorrespondenceState(), PostCorrespondenceState(), 
        PostCorrespondenceState(), PostCorrespondenceState()],
    ]
    for idx, test_dominos in enumerate(dominos):
      test_result = map(self.Dominos._CatDomino, states, test_dominos)
      self.assertSequenceEqual([x.seqs for x in expected[idx]], [x.seqs for x in test_result])
      self.assertSequenceEqual([x.history for x in expected[idx]], [x.history for x in test_result])

  def testNeighbors(self):
    states = [
      PostCorrespondenceState(("", ""),[]), PostCorrespondenceState(("", "ca"),[1]),
      PostCorrespondenceState(("b", ""),[3]), PostCorrespondenceState(("c", ""),[3, 2]), 
      PostCorrespondenceState(("", "a"),[3, 2, 1]), PostCorrespondenceState(("", "b"),[3, 2, 1, 4])
    ]
    expected = [
      [PostCorrespondenceState(("", "ca"),[1]), PostCorrespondenceState(("b", ""),[3])],
      [PostCorrespondenceState(("", "acca"),[1, 1])],
      [PostCorrespondenceState(("c", ""),[3, 2]), PostCorrespondenceState(("bb", ""),[3, 3])],
  ...
  def testAssert(self):
    states = [
      PostCorrespondenceState(("", ""),[1]), PostCorrespondenceState(("", "a"),[1]),
      PostCorrespondenceState(("a", ""),[1]), PostCorrespondenceState(("a", "a"),[1]), 
      PostCorrespondenceState(("b", "a"),[1]), PostCorrespondenceState(("ab", "a"),[1]), 
  ...
  def testDominoFunctional(self):
    expected = [PostCorrespondenceState(("", "ca"),[1]), PostCorrespondenceState(("b", ""),[3])]
    test_result = self.Dominos.Neighbors(self.Dominos.start_point)
    self.assertSequenceEqual([x.seqs for x in expected], [x.seqs for x in test_result])
    self.assertSequenceEqual([x.history for x in expected], [x.history for x in test_result])
